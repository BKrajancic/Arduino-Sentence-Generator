...
class Sentence_Generator
{
  public:
    Sentence_Generator()
      : sizer(new Monospace_Sizer<String>())
      , elipsis(Word("..."))
      , overflow(Word("-"))
    {
        macros[0] = new Remove_Empty<Word, Sentence, Sentence_Iterator>();
        macros[1] = new No_Space_Begin<Word, Sentence, Sentence_Iterator>();
        macros[2] = new No_Space_End<Word, Sentence, Sentence_Iterator>();
        g = Grammar();
        //shuffle();
    }
    ...
    */
    ...
    Sentence run_macros(Sentence input) const
    {
        ...
        for(auto i = 0; i < 3; ++i)
        {
            ...
            {
                if (macros[i]->match(*it))
                {
                    macros[i]->execute(input, it);
                    goto start_macros;
                }
            }
        }
    ...
    Sentence get_screen(const unsigned int width_per_row, const unsigned int rows_per_screen)
    {
        ...
        if (size_remaining() != 0u)
        {
          ...
          else if (sizer->word_fits(stack.back(), width_per_row) != true)
          {
              ...
              {
                  removed = substring(next, length(next) - 1, 1) + removed;
                  next = substring(next, 0, length(next) - 1);
              }
              stack.pop_back();
  ...
  private:
    ...
    */
    ...
    Grammar g;
    Sentence stack;
    Macro<Word, Sentence, Sentence_Iterator>* macros[3];
};
